...
def test_recommend_wls_midband_interior():
...
def test_recommend_wls_multi_band_pinned():
  ...
  assert rec.triple[0] == pytest.approx(ground_truth[0], abs=0.01)
  assert rec.triple[1] == pytest.approx(ground_truth[1], abs=0.01)
  assert rec.triple[2] == pytest.approx(ground_truth[2], abs=0.01)


def test_recommend_wls_multi_band_capped_moves_toward_truth():
  ground_truth = (1.30, 1.45, 1.10)
  events = _build_pinned_events(ground_truth)
  bs = _band_stats(low_r=events[0].tracking_ratio,
                   mid_r=events[1].tracking_ratio,
...
def test_recommend_15x_cap_down():
...
def test_recommend_oscillation_guard():
...
def test_recommend_hold_inside_band():
...
def test_recommend_back_off_overshoot():
...
def test_recommend_insufficient_samples():
...
def test_recommend_low_speed_excluded():
  ...
  assert rec_clean.triple == pytest.approx(rec_filtered.triple, abs=1e-9)
...
def test_verdicts_classification():
...
from openpilot.tools.sunnypilot.kp_tuner.recommend import recommend_joint
...
def test_joint_two_bucket_recovery():
  ...
  for got, truth in zip(rec.triple, ground_truth, strict=True):
    assert got == pytest.approx(truth, abs=0.02), f"got {got}, truth {truth}"


def test_joint_single_bucket_matches_recommend():
  ...
  for joint_v, single_v in zip(joint_rec.triple, per_bucket_rec.triple, strict=True):
    assert joint_v == pytest.approx(single_v, abs=0.005), \
      f"joint {joint_v} vs single {single_v}"


def test_joint_insufficient_events_returns_hold():
  rec = recommend_joint([], min_events=4)
  assert rec.hold is True
  assert "needs more data" in rec.rationale


def test_joint_clamps_to_kp_ui_range():
...
def test_joint_oversteer_bucket_pushes_recommendation_down():
