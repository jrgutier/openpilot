...
def test_synthetic_2_bucket_walk_discovers_2_buckets(tmp_path: Path, monkeypatch):
...
def test_synthetic_4_bucket_walk_discovers_4_buckets(tmp_path: Path, monkeypatch):
...
def test_combined_html_has_per_bucket_sections(tmp_path: Path, monkeypatch):
  events = [_evt(20.0, 0.9), _evt(22.0, 0.95)]
  monkeypatch.setattr(
    report, "_partition_segments_by_bucket",
    lambda _d, *, use_cache: (
...
def test_bucket_filter_restricts_to_one_model(tmp_path: Path, monkeypatch):
  events = [_evt(20.0, 0.9), _evt(22.0, 0.95)]
  monkeypatch.setattr(
    report, "_partition_segments_by_bucket",
    lambda _d, *, use_cache: (
...
def test_bucket_id_is_deterministic_and_human_readable():
...
def _two_bucket_pipeline(tmp_path: Path, monkeypatch, *, joint: bool, out_html: Path):
...
def test_joint_html_section_present_when_flag_set(tmp_path: Path, monkeypatch):
  out_html = tmp_path / "report.html"
  rc = _two_bucket_pipeline(tmp_path, monkeypatch, joint=True, out_html=out_html)
  assert rc == report.EXIT_OK
  text = out_html.read_text()
...
def test_joint_html_section_absent_when_flag_unset(tmp_path: Path, monkeypatch):
  out_html = tmp_path / "report.html"
  rc = _two_bucket_pipeline(tmp_path, monkeypatch, joint=False, out_html=out_html)
  assert rc == report.EXIT_OK
  text = out_html.read_text()
...
def test_joint_never_writes_to_state(tmp_path: Path, monkeypatch):
