...
class TestLatControlTorqueKpLoad(OpenpilotTestCase):

  def test_load_kp_multipliers_defaults_when_none(self):
    assert _load() == [1.0, 1.0, 1.0]

  def test_load_kp_multipliers_happy_path(self):
    assert _load(KpLowSpeed="0.7", KpMidSpeed="0.85", KpHighSpeed="0.95") == [0.7, 0.85, 0.95]

  def test_load_kp_multipliers_bad_string_falls_back_to_default(self):
    assert _load(KpLowSpeed="not-a-float") == [1.0, 1.0, 1.0]

  def test_load_kp_multipliers_clamps_out_of_range_values(self):
