...
def _resolve_log_root(override: str | None) -> Path:
  ...
  for p in DEFAULT_LOG_ROOTS:
    if p.is_dir():
      return p
  raise FileNotFoundError(f"no log root found; tried {', '.join(str(p) for p in DEFAULT_LOG_ROOTS)}. pass --log-root explicitly.")
...
def main() -> int:
  ...
  ap.add_argument("--log-root", default=None, help="override log root (default: ~/sunnypilot-logs on dev, /data/media/0/realdata on comma)")
  args = ap.parse_args()
