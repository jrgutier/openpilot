...
class TestCache(OpenpilotTestCase):
  """Each test gets its own isolated `XDG_CACHE_HOME` and reset cache stats —
  the pytest-version's autouse `_isolated_cache_dir` fixture, translated into
  setUp() since this harness has no autouse-fixture concept."""

  def setUp(self):
    super().setUp()
    self._xdg_tmp = tempfile.TemporaryDirectory()
    self.addCleanup(self._xdg_tmp.cleanup)
    patcher = mock.patch.dict(os.environ, {"XDG_CACHE_HOME": self._xdg_tmp.name})
    patcher.start()
  ...
  def test_cache_miss_then_hit(self, tmp_path: Path):
    ...
    assert cached.events[0].tracking_ratio == approx(0.85)

  def test_cache_miss_no_file(self, tmp_path: Path):
    rlog = tmp_path / "does-not-exist.rlog.zst"
    assert _cache.read_cached(rlog) is None

  def test_schema_version_mismatch_silent_miss(self, tmp_path: Path):
    rlog = _make_rlog(tmp_path)
  ...
  def test_extractor_version_mismatch_silent_miss(self, tmp_path: Path):
    rlog = _make_rlog(tmp_path)
    _cache.write_cached_atomic(rlog, None, (1.0, 1.0, 1.0), True, [])
    cache_file = _cache._cache_file(rlog)
    data = json.loads(cache_file.read_text())
  ...
  def test_salt_mismatch_silent_miss(self, tmp_path: Path, monkeypatch):
    rlog = _make_rlog(tmp_path)
    _cache.write_cached_atomic(rlog, None, (1.0, 1.0, 1.0), True, [])
  ...
  def test_mtime_drift_silent_miss(self, tmp_path: Path):
    rlog = _make_rlog(tmp_path)
  ...
  def test_size_drift_silent_miss(self, tmp_path: Path):
  ...
  def test_corrupt_json_silent_miss(self, tmp_path: Path):
    rlog = _make_rlog(tmp_path)
    _cache.write_cached_atomic(rlog, None, (1.0, 1.0, 1.0), True, [])
    cache_file = _cache._cache_file(rlog)
  ...
  def test_write_failure_preserves_analysis(self, tmp_path: Path, monkeypatch, caplog):
  ...
  def test_monkeypatched_salt_busts_cache(self, tmp_path: Path, monkeypatch):
