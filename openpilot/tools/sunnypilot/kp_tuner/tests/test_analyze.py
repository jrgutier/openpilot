...
class TestAnalyze(OpenpilotTestCase):
  # -------------------------------------------------------------------------
  # Helpers / pure functions
  # -------------------------------------------------------------------------

  def test_interp_weights_endpoints(self):
    assert interp_weights(0.0) == (1.0, 0.0, 0.0)
    assert interp_weights(6.7) == (1.0, 0.0, 0.0)
    assert interp_weights(15.6) == approx((0.0, 1.0, 0.0))
    assert interp_weights(33.5) == approx((0.0, 0.0, 1.0))
    assert interp_weights(50.0) == (0.0, 0.0, 1.0)

  def test_interp_weights_interior_sums_to_one(self):
    for v in [4.0, 11.0, 20.0, 30.0]:
      w = interp_weights(v)
      assert sum(w) == approx(1.0)

  def test_band_for_vego(self):
  ...
  def test_current_kp_working_uses_interp(self):
  ...
  def test_curve_entry_threshold_speed_aware(self):
  ...
  ], names=["peak", "expected_events"])
  def test_curve_entry_threshold_edges(self, peak, expected_events):
  ...
  def test_partition_and_summarize(self):
    events_low = []
    events_mid = []
    ...
    assert bs.mid.median_ratio == approx(0.9, abs=0.02)

  # -------------------------------------------------------------------------
  # NN-FF detection
  # -------------------------------------------------------------------------

  def test_nn_ff_detection_dual_check(self):
  ...
  def test_read_kp_triple_dict_path(self):
  ...
  def test_read_kp_triple_capnp_entries_path(self):
    init = _InitDataCapnp({"KpLowSpeed": b"1.2", "KpMidSpeed": b"1.0", "KpHighSpeed": b"0.8"})
    assert read_kp_triple_from_init_data(init) == approx((1.2, 1.0, 0.8))

  def test_read_kp_triple_missing_keys_default_one(self):
    assert read_kp_triple_from_init_data(_InitData({})) == (1.0, 1.0, 1.0)

  def test_read_kp_triple_bad_value_falls_back_to_one(self):
  ...
  def test_read_kp_triple_clamps_out_of_range(self):
  ...
  def test_read_kp_triple_returns_none_when_no_init_data(self):
    assert read_kp_triple_from_init_data(None) is None

  def test_read_kp_triple_returns_none_when_no_params_attr(self):
  ...
  def test_extract_synthetic_oversteer_event(self):
    ...
    events = extract_curve_events(msgs)
    assert len(events) == 1
    assert events[0].tracking_ratio == approx(1.15, abs=0.02)
  ...
  def test_summarize_mixed_directional_counts(self):
