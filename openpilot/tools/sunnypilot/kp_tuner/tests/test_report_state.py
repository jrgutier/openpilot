...
from openpilot.tools.sunnypilot.kp_tuner.analyze import BandStat, BandStats, CurveEvent
...
class TestReportState(OpenpilotTestCase):
  # ---------------------------------------------------------------------------
  ...
  def test_state_atomic_write_no_tmp_left(self, tmp_path: Path):
    sp = tmp_path / "state.json"
    state = report._empty_state()
    report.save_state_atomic(state, sp)
    assert sp.exists()
  ...
  def test_v2_to_v3_migration(self, tmp_path: Path):
  ...
  def test_last_known_safe_persists(self, tmp_path: Path):
    sp = tmp_path / "state.json"
    state = report._empty_state()
  ...
  def test_last_known_safe_does_not_advance_on_hold(self):
    state = report._empty_state()
    bid = report.bucket_id_for("M", (1.0, 1.0, 1.0))
  ...
  def test_revert_flag_returns_lks_and_does_not_mutate(self, tmp_path: Path, capsys):
    sp = tmp_path / "state.json"
    state = report._empty_state()
    bid = report.bucket_id_for("M", (1.0, 1.0, 1.0))
    report.append_iteration(state, (1.0, 1.0, 1.0), _bs(),
  ...
  def test_revert_flag_no_state(self, tmp_path: Path, capsys):
  ...
  def test_revert_ambiguity_multiple_buckets(self, tmp_path: Path, capsys):
  ...
  def test_revert_with_filter_resolves_one_bucket(self, tmp_path: Path, capsys):
    sp = tmp_path / "state.json"
    state = report._empty_state()
    for model, kp in (("A", (1.0, 1.0, 1.0)), ("B", (0.7, 0.8, 0.9))):
      bid = report.bucket_id_for(model, kp)
  ...
  def test_revert_with_filter_zero_matches(self, tmp_path: Path, capsys):
    sp = tmp_path / "state.json"
    state = report._empty_state()
  ...
  def test_nn_ff_abort_when_param_on_and_real_model(self, tmp_path: Path, capsys, monkeypatch):
  ...
  def test_nn_ff_proceeds_when_param_on_but_mock(self, tmp_path: Path, monkeypatch):
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    init_on = _InitData({"NeuralNetworkLateralControl": b"1"})
  ...
  def test_bucket_filter_zero_matches_full_run(self, tmp_path: Path, monkeypatch, capsys):
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    monkeypatch.setattr(
      report, "_partition_segments_by_bucket",
  ...
  def test_parse_triple_valid(self):
    assert report._parse_triple("1.0,1.5,2.0") == (1.0, 1.5, 2.0)

  def test_parse_triple_wrong_arity(self):
    with self.assertRaises(argparse.ArgumentTypeError):
      report._parse_triple("1.0,2.0")

  def test_parse_triple_non_numeric(self):
    with self.assertRaises(argparse.ArgumentTypeError):
  ...
  def test_parser_help_includes_revert(self):
  ...
  def test_e2e_no_buckets_exits_usage(self, tmp_path: Path, monkeypatch, capsys):
  ...
  def test_render_html_under_over_column(self):
    bs = BandStats(
      low=None,
      mid=None,
      high=BandStat(
  ...
  def test_console_directional_text(self, tmp_path: Path, capsys, monkeypatch):
