...
The VDM_UserAdasRequest==2 raw CAN decode below is DBC-level and unchanged
across branches. What DOES differ on this branch: carstate_ext.py reports the
UP_2 edge as a `ButtonEvent(type=ButtonType.altButton2)` (not `.cancel`), and
car_specific.py's rivian branch turns that into `EventNameSP.lkasDisable` to
force the full MADS disengage this script is checking for (see
opendbc_repo/opendbc/sunnypilot/car/rivian/carstate_ext.py:~110-116 and
openpilot/sunnypilot/selfdrive/car/car_specific.py:~66-71). The onroadEvents
keyword filter below is updated to match "lkasdisable" instead of "cancel"
accordingly; the pass/fail metric itself (selfdriveState.enabled edges) is
unaffected by the encoding change.
...
for rid in routes:
    ...
    for msg in lr:
      ...
      if which == "selfdriveState":
        ...
        for ev in msg.onroadEvents:
          n = str(ev.name)
          if any(k in n.lower() for k in ("cancel", "buttoncancel", "stalk", "disengage", "lkasdisable")):
            stalk_events[n] += 1
  ...
  for press_t in up2_times:
    ...
    matching = next((dt for dt in selfdrive_drop_times if dt not in used_drops and 0 <= dt - press_t <= DISENGAGE_WINDOW_NS), None)
