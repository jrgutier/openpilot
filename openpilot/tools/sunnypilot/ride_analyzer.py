...
# LongitudinalPersonality enum (cereal/log.capnp:144-148) only has 3 ordinals on this branch —
# "Very Aggressive" is NOT ordinal 3. It's a fork extension published as
# `longitudinalPlanSP.veryAggressive` (cereal/custom.capnp LongitudinalPlanSP), true only when the
# toggle is enabled AND personality==aggressive (see selfdrive/controls/lib/longitudinal_planner.py).
# We synthesize the old-style ordinal-3 bucket below so the rest of this file (buckets, gate,
# top-10 export) can stay keyed 0-3 unchanged.
...
# veryAggressive's 0.8 is VERY_AGGRESSIVE_T_FOLLOW in
# sunnypilot/selfdrive/controls/lib/longitudinal_planner.py, not long_mpc.py.
...
SEG_FIELDS = [
  ...
  "long_jerk_p99",
  "a_err_rms",
  "a_err_p95",  # actuators.accel - aEgo (when longActive)
...
def analyze_segment(seg_path: Path) -> dict | None:
  ...
  # longitudinalPlanSP.veryAggressive — fork-extension flag, latched (see PERSONALITY_NAME note above)
  t_lpsp, va_flag = [], []
  ...
  try:
    for msg in LogReader(str(rlog)):
      ...
      if w == "carParams":
        ...
        t_lpsp.append(msg.logMonoTime * 1e-9)
        va_flag.append(bool(msg.longitudinalPlanSP.veryAggressive))
      elif w == "radarState":
        t_radar.append(msg.logMonoTime * 1e-9)
        lo = msg.radarState.leadOne
        # LeadData.status (Int8) was deprecated upstream in favor of .present (Bool);
        # this branch's log.capnp only has .present.
        lead_status.append(lo.present)
        lead_dRel.append(lo.dRel if lo.present else 0.0)
        lead_vLead.append(lo.vLead if lo.present else 0.0)
  ...
  if len(cur_curv_arr):
    cur_curv_arr = cur_curv_arr[:n]
  if len(actuator_accel_arr):
    actuator_accel_arr = actuator_accel_arr[:n]
  ...
  # Fold the veryAggressive flag into the ordinal-3 synthetic bucket (see PERSONALITY_NAME note
  # above): true only where the fork flag was latched AND the sample's personality is aggressive.
  if t_lpsp:
    t_lpsp_arr = np.asarray(t_lpsp)
    idx = np.searchsorted(t_lpsp_arr, t, side="right") - 1
    idx = np.clip(idx, 0, len(t_lpsp_arr) - 1)
    va_flag_per_sample = np.asarray(va_flag, dtype=bool)[idx]
    personality_per_sample = np.where((personality_per_sample == 0) & va_flag_per_sample, 3, personality_per_sample).astype(np.int8)
...
def _agg(rows: list[dict]) -> dict:
  ...
  for k in ("a_p95_decel", "a_p95_accel", "long_jerk_p99", "a_err_p95", "lat_a_p95", "lat_jerk_p99", "curv_err_p95", "cum_lag_p95_ms", "v_max_mps"):
    out[k] = round(smax(k), 6)
  for k in ("n_disengage", "n_steer_override", "n_brake_override", "n_gas_override", "n_hard_decel"):
    out[k] = ssum(k)
  return out
...
def _markdown_report(seg_rows: list[dict], route_rows: list[dict]) -> str:
  ...
  lines.append(
    "| route | min | v̄ (m/s) | enabled | lat jerk RMS | lat jerk p99 | long jerk RMS | long jerk p99 | curv err p95 | a err RMS | hard decel | disengage |"
  )
  lines.append("|---|---|---|---|---|---|---|---|---|---|---|---|")
  for r in rs:
    lines.append(
  ...
  for r in top:
    lines.append(
      f"| `{r['route']}` | {r['seg']} | {r['duration_s']:.1f} | {r['v_mean_mps']:.1f} | {r['long_jerk_p99']:.3f} | {r['a_err_p95']:.3f} | {r['n_hard_decel']} |"
    )
  ...
  lines.append("|---|---|---|---|---|---|")
  for r in top:
    lines.append(f"| `{r['route']}` | {r['seg']} | {r['duration_s']:.1f} | {r['v_mean_mps']:.1f} | {r['a_err_rms']:.3f} | {r['a_err_p95']:.3f} |")

  lines.append("")
  lines.append("## Notes / column glossary\n")
  lines.append(
    "- **lat jerk** (m/s³): time-derivative of v²·κ, where κ is `carControl.currentCurvature`. RMS is overall smoothness; p99 catches transient yanks.\n" +
...
def _aggregate_personality_buckets(segment_results: list[dict]) -> dict:
    ...
    for p, bucket in sr["buckets"].items():
      agg = by_pers.setdefault(
        p,
        {
          ...
          "t_headway_chunks": [],
          "closing_rate_chunks": [],
          "frac_lead_close_weighted_sum": 0.0,
          "frac_lead_close_weight": 0.0,
        },
      )
      ...
      if bucket["jerk"].size > 0:
        events[p].append(
          (
            seg["route"],
            seg["seg"],
            bucket["peak_offset_s"],
            bucket["peak_jerk_abs"],
            seg.get("v_mean_mps", float("nan")),
          )
        )
  out = {}
  for p, agg in by_pers.items():
    ...
    out[p] = {
      ...
      "n_disengage": int(sum(sr["seg_row"]["n_disengage"] for sr in segment_results if sr["seg_row"].get("personality_dominant") == PERSONALITY_NAME[p])),
      ...
      "frac_lead_close": round(agg["frac_lead_close_weighted_sum"] / agg["frac_lead_close_weight"], 4) if agg["frac_lead_close_weight"] > 0 else float("nan"),
    }
...
def _markdown_report_by_personality(personality_agg: dict) -> str:
  ...
  lines.append(
    "Sample-level personality attribution: each metric is computed over samples where " +
    "`selfdriveState.personality == X` AND `carControl.longActive == True`. Lead metrics " +
    "additionally require `radarState.leadOne.present == True` and `vEgo > 1 m/s`.\n"
  )

  lines.append("\n## Per-personality summary\n")
  lines.append(
    "| personality | t_follow setpoint | minutes | a RMS | long jerk RMS | long jerk p99 | " +
  ...
  lines.append("\n## Notes\n")
  lines.append(
    "- **Sample-level attribution**: every metric in the per-personality table is computed over " +
    "the sample set where the attached personality was active AND longitudinal control was " +
    "active. Segment-level mode attribution is *not* used."
  )
  lines.append(
    "- **frac_lead_close** is a proxy: fraction of lead-present time where " +
    "`dRel < t_follow_setpoint × vEgo`. It indicates that headway stress is plausibly present, " +
    "not that it caused the jerk."
  )
  lines.append(
    "- **Top-10 events** are one-per-(route, segment) — peak |jerk| within each veryAggressive " +
    "bucket of each segment. The `offset (s)` is the wall-clock seconds from segment start."
  )
  return "\n".join(lines)
...
def main():
  ...
  top10_payload = {
    ...
    "events": [{"route": e[0], "seg": e[1], "offset_s": (None if math.isnan(e[2]) else e[2]), "peak_jerk_abs": e[3], "v_mean_mps": e[4]} for e in va_events],
  }
